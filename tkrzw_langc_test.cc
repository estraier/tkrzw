...
TEST(LangCTest, Index) {
  ...
  EXPECT_EQ(3, tkrzw_index_count(index));
  EXPECT_TRUE(tkrzw_index_clear(index));
  EXPECT_EQ(0, tkrzw_index_count(index));
  EXPECT_TRUE(tkrzw_index_is_writable(index));
  EXPECT_TRUE(tkrzw_index_add(index, "first", -1, "1", -1));
  EXPECT_TRUE(tkrzw_index_add(index, "second", -1, "2", -1));
  EXPECT_TRUE(tkrzw_index_add(index, "third", -1, "3", -1));
  ...
  EXPECT_EQ("first", std::string_view(key_ptr, key_size));
  EXPECT_EQ("1", std::string_view(value_ptr, value_size));
  free(value_ptr);
  free(key_ptr);
  ...
  tkrzw_index_iter_next(iter);
  ASSERT_TRUE(tkrzw_index_iter_get(iter, &key_ptr, &key_size, &value_ptr, &value_size));
  EXPECT_EQ("third", std::string_view(key_ptr, key_size));
  EXPECT_EQ("3", std::string_view(value_ptr, value_size));
  free(value_ptr);
  free(key_ptr);
  tkrzw_index_iter_next(iter);
  ASSERT_FALSE(tkrzw_index_iter_get(iter, &key_ptr, &key_size, &value_ptr, &value_size));
  tkrzw_index_iter_last(iter);
  ASSERT_TRUE(tkrzw_index_iter_get(iter, &key_ptr, &key_size, &value_ptr, &value_size));
  EXPECT_EQ("third", std::string_view(key_ptr, key_size));
  EXPECT_EQ("3", std::string_view(value_ptr, value_size));
  free(value_ptr);
